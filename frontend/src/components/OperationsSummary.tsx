...
function MetricCard({ label, value, detail, tone }: { label: string; value: string | number | null; detail: string; tone: string }) {
  return (
    <div className="min-w-0 rounded-lg border border-[#1a435f] bg-[#07111f]/94 px-3 py-2.5 shadow-lg backdrop-blur">
...
export function OperationsSummary({ simulation, decision, district }: OperationsSummaryProps) {
  ...
  return (
    <section className="absolute bottom-5 left-5 right-5 z-[1000] rounded-xl border border-[#1a435f] bg-[#07111f]/94 p-3 shadow-2xl backdrop-blur" aria-label="Operations summary">
      <div className="mb-2 flex items-center justify-between gap-3">
        <div>
          <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-[#22c7ff]">Operations Summary</p>
        ...
        <span className="hidden rounded-full border border-[#1a435f] px-2 py-1 text-[9px] font-semibold uppercase tracking-wider text-slate-400 sm:inline-flex">Live incident state</span>
      </div>
