...
export function DecisionController({ district }: DecisionControllerProps) {
  ...
  useEffect(() => {
    if (!incidentZoneId) {
      setDecision(null); setReport(null); setDecisionError(null); setReportError(null); return;
    }
    ...
    fetchDecisionBundle(zone.id, scenario, zone.elevation_m, zone.drainage_score)
      ...
      .catch((error) => { console.error(error); if (!cancelled) { setDecision(null); setDecisionError(friendlyError(error, "Unable to calculate the decision bundle. Please try again.")); } })
  ...
  const generateReport = () => {
    ...
    fetchReport(decision).then(setReport).catch((error) => { console.error(error); setReportError(friendlyError(error, "The incident report could not be generated.")); }).finally(() => setLoadingReport(false));
  };
