...
export function DecisionController({ district }: DecisionControllerProps) {
  ...
  return <div className="relative"><OperationalMap zones={district.zones} hospitals={district.hospitals} shelters={district.shelters} rescueTeams={district.rescue_teams} hospitalRoute={decision?.hospital_route} shelterRoute={decision?.shelter_route} teamRoute={decision?.team_route} simulation={simulation} center={district.metadata.center} selectedIds={selectedIds} onEntityClick={handleEntityClick} />
    <div className="absolute left-4 top-4 z-[1000] w-60 rounded-xl border border-[#1a435f] bg-[#07111f]/95 p-3.5 shadow-2xl backdrop-blur-md"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-water">Scenario</p><h3 className="mt-1 text-sm font-semibold text-white">Rainfall simulation</h3><div className="mt-3 grid grid-cols-3 gap-1 rounded-lg bg-slate-900/80 p-1">{scenarios.map((value) => <button key={value} type="button" disabled={loadingSimulation || loadingDecision} onClick={() => { setIncidentZoneId(null); setScenario(value); }} className={`rounded-md px-2 py-2 text-[10px] font-bold uppercase ${scenario === value ? "bg-[#22c7ff] text-[#03121d]" : "text-slate-300 hover:bg-slate-800"}`}>{value}</button>)}</div>{simulationError ? <div className="mt-3 rounded-md border border-rose-500/30 bg-rose-500/5 p-2 text-[11px] text-rose-300">{simulationError}</div> : <p className="mt-3 text-[11px] text-slate-400">Click any <strong className="text-sky-400">zone</strong> on the map to run the decision engine.</p>}</div>
    <div className="absolute left-4 top-[190px] z-[1000] w-[310px] max-h-[68%] overflow-y-auto rounded-xl border border-[#1a435f] bg-[#07111f]/96 p-3.5 shadow-2xl backdrop-blur-md"><div className="mb-3 flex items-center justify-between"><h3 className="text-xs font-bold uppercase tracking-[0.18em] text-water">Decision Engine</h3>{incidentZoneId && <button type="button" onClick={() => setIncidentZoneId(null)} className="text-[11px] font-semibold text-slate-400 hover:text-white">CLEAR</button>}</div>{!incidentZoneId ? <p className="text-sm text-slate-400">No incident selected. Click a flood zone to begin analysis.</p> : loadingDecision ? <div className="space-y-2"><p className="animate-pulse text-sm text-slate-300">Analyzing incident...</p><p className="text-xs text-slate-500">Calculating safe routes and operational recommendations.</p></div> : decisionError ? <div className="rounded-lg border border-rose-500/30 bg-rose-500/5 p-3 text-sm text-rose-300"><p>{decisionError}</p><button type="button" onClick={() => { const id = incidentZoneId; setIncidentZoneId(null); window.setTimeout(() => setIncidentZoneId(id), 0); }} className="mt-3 rounded-md border border-rose-400/30 px-3 py-1.5 text-xs font-semibold text-rose-200 hover:bg-rose-500/10">Retry analysis</button></div> : decision ? <div className="space-y-4"><div className="rounded-lg border border-[#23415a] bg-[#0b1a2a]/90 p-3"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Flood Prediction</p><div className="mt-1 flex items-end gap-2"><span className="text-2xl font-black text-white">{decision.simulation.severity_score.toFixed(0)}<span className="text-sm text-slate-500">/100</span></span><span className="mb-1 rounded bg-slate-900 px-2 py-0.5 text-[10px] font-semibold uppercase text-[#ff5964]">{decision.prediction.confidence} confidence</span></div><p className="mt-2 text-xs leading-5 text-slate-400">{decision.prediction.explanation}</p><div className="mt-3 grid grid-cols-2 gap-2">{Object.entries(decision.prediction.input_factors).map(([factor, value]) => <div key={factor} className="rounded-md border border-slate-700 bg-slate-900/60 px-2 py-2"><p className="text-[9px] uppercase tracking-wider text-slate-500">{factor.replace(/_/g, " ")}</p><p className="mt-1 text-xs font-semibold text-slate-200">{value}</p></div>)}</div></div>{decision.team_allocation.selected_team && <RecommendationPanel title="Dispatched Rescue Team" accent="text-[#22d3a7]" candidate={decision.team_allocation.selected_team} empty="No team assigned" explanation={decision.team_allocation.explanation} factors={decision.team_allocation.selected_team.reasoning_factors} details={`${decision.team_allocation.selected_team.route_distance_km} km away · ETA ${decision.team_allocation.selected_team.estimated_travel_time_minutes} mins`} />}<RecommendationPanel title="Recommended Hospital" accent="text-rose-400" candidate={decision.hospital_recommendation.selected_hospital} empty="No suitable hospital" explanation={decision.hospital_recommendation.explanation} factors={decision.hospital_recommendation.selected_hospital?.reasoning_factors} details={decision.hospital_recommendation.selected_hospital ? `${decision.hospital_recommendation.selected_hospital.route_distance_km} km away · ${decision.hospital_recommendation.selected_hospital.available_capacity} beds` : ""} /><RecommendationPanel title="Evacuation Shelter" accent="text-[#ffc233]" candidate={decision.shelter_recommendation.selected_shelter} empty="No suitable shelter" explanation={decision.shelter_recommendation.explanation} factors={decision.shelter_recommendation.selected_shelter?.reasoning_factors} details={decision.shelter_recommendation.selected_shelter ? `${decision.shelter_recommendation.selected_shelter.route_distance_km} km away · ${decision.shelter_recommendation.selected_shelter.available_capacity} spaces` : ""} />{reportError && <div className="rounded-lg border border-rose-500/30 bg-rose-500/5 p-3 text-sm text-rose-300"><p>{reportError}</p><button type="button" onClick={generateReport} className="mt-3 rounded-md border border-rose-400/30 px-3 py-1.5 text-xs font-semibold text-rose-200 hover:bg-rose-500/10">Retry report</button></div>}{!report && !reportError ? <button type="button" onClick={generateReport} disabled={loadingReport} className="w-full rounded-lg bg-water py-2 text-sm font-semibold text-slate-950 disabled:opacity-50">{loadingReport ? "Generating..." : "Generate Incident Report"}</button> : null}{report && <div className="rounded-lg border border-[#22c7ff]/30 bg-[#22c7ff]/5 p-3"><div className="flex items-center justify-between gap-2"><p className="text-[10px] font-bold uppercase tracking-wider text-water">AI Incident Report</p><div className="flex gap-2"><button type="button" onClick={copyReport} className="rounded-md border border-slate-600 px-2 py-1 text-[10px] font-semibold text-slate-300 hover:bg-slate-800">Copy</button><button type="button" onClick={downloadReport} className="rounded-md border border-[#22c7ff]/40 px-2 py-1 text-[10px] font-semibold text-[#22c7ff] hover:bg-water/10">Download</button></div></div><pre className="mt-2 whitespace-pre-wrap font-sans text-xs text-slate-300">{report.report_content}</pre>{copyStatus && <p className="mt-2 text-[10px] text-slate-500">{copyStatus}</p>}</div>}</div> : null}</div>
