...
export function DecisionController({ district }: DecisionControllerProps) {
  ...
  useEffect(() => {
    ...
    fetchSimulation(scenario).then((result) => { if (!cancelled) setSimulation(result); })
      .catch((error) => { console.error(error); if (!cancelled) { setSimulation(null); setSimulationError("Scenario data could not be loaded. Retry or choose another scenario."); } })
      .finally(() => { if (!cancelled) setLoadingSimulation(false); });
    return () => { cancelled = true; };
  }, [scenario]);

  useEffect(() => {
    if (!incidentZoneId) { setDecision(null); setReport(null); setDecisionError(null); setReportError(null); return; }
    const zone = district.zones.find((z) => z.id === incidentZoneId);
    if (!zone) { setDecision(null); setDecisionError("The selected incident zone is no longer available."); return; }
    let cancelled = false;
    setLoadingDecision(true); setReport(null); setDecisionError(null); setReportError(null);
  ...
  const generateReport = () => { if (!decision) return; setLoadingReport(true); setReportError(null); fetchReport(decision).then(setReport).catch((error) => { console.error(error); setReportError("Report generation failed. Please try again."); }).finally(() => setLoadingReport(false)); };
  ...
  return <div className="relative">
    ...
    <div className="absolute left-5 top-5 z-[1000] w-64 rounded-xl border border-slate-700 bg-ink/95 p-4 shadow-xl backdrop-blur">
      ...
      <div className="grid grid-cols-3 gap-1 rounded-lg bg-slate-900/80 p-1">{scenarios.map((value) => <button key={value} type="button" disabled={loadingSimulation} onClick={() => setScenario(value)} className={`rounded-md px-2 py-2 text-[10px] font-bold uppercase transition disabled:cursor-wait disabled:opacity-50 ${scenario === value ? "bg-water text-slate-950" : "text-slate-300 hover:bg-slate-800"}`}>{value}</button>)}</div>
      ...
      {simulationError && <p className="mt-2 text-[10px] leading-4 text-rose-300">{simulationError}</p>}
    </div>
    <div className="absolute right-5 top-5 z-[1000] w-[340px] max-h-[85vh] overflow-y-auto rounded-xl border border-slate-700 bg-ink/95 p-4 shadow-xl backdrop-blur scrollbar-hide">
      ...
      {!incidentZoneId ? <p className="text-sm text-slate-400">No incident selected. Click a flood zone to begin analysis.</p> : loadingDecision ? <p className="animate-pulse text-sm text-slate-400">Analyzing incident parameters and calculating optimal routes...</p> : decisionError ? <div className="rounded-lg border border-rose-500/30 bg-rose-500/5 p-3 text-sm text-rose-300">{decisionError}</div> : decision ? <div className="space-y-4">
        ...
        {!report ? <><button type="button" onClick={generateReport} disabled={loadingReport} className="w-full rounded-lg bg-water py-2 text-sm font-semibold text-slate-950 transition hover:bg-sky-400 disabled:opacity-50">{loadingReport ? "Generating..." : "Generate Incident Report"}</button>{reportError && <p className="text-[11px] text-rose-300">{reportError}</p>}</> : <div className="rounded-lg border border-water/30 bg-water/5 p-3"><p className="text-[10px] font-bold uppercase tracking-wider text-water">AI Incident Report</p><pre className="mt-2 whitespace-pre-wrap font-sans text-xs text-slate-300">{report.report_content}</pre><ReportActions content={report.report_content} /></div>}
      </div> : null}
    </div>
    <OperationsSummary simulation={simulation} decision={decision} />
