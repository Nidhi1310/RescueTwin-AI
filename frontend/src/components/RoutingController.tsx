    ...
    <div className="relative">
        ...
        {simulation && (
          <div className="mt-3 grid grid-cols-2 gap-2 text-[11px]">
            <div className="rounded-lg border border-white/10 bg-slate-900/60 p-2">
              ...
              <p className="mt-1 font-bold text-white">{simulation.severity_score.toFixed(1)}</p>
            </div>
            <div className="rounded-lg border border-white/10 bg-slate-900/60 p-2">
              <p className="text-slate-400">Blocked</p>
              <p className="mt-1 font-bold text-rose-400">{simulation.blocked_roads.length}</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="absolute right-5 top-5 z-[1000] w-72 rounded-xl border border-slate-700 bg-ink/95 p-4 shadow-xl backdrop-blur">
        <div className="mb-3 flex items-center justify-between">
          ...
          {(startId || endId) && (
            <button type="button" onClick={resetRouting} className="text-[11px] font-semibold text-slate-400 hover:text-white">
              CLEAR
            </button>
          )}
        </div>
        ...
        <div className="space-y-2 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="truncate font-medium text-white">{startId || "Select point"}</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="h-2 w-2 rounded-full bg-blue-400" />
            <span className="text-slate-300">End:</span>
            <span className="truncate font-medium text-white">{endId || "Select point"}</span>
          </div>
        </div>
        {route && (
