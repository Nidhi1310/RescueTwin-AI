...
def calculate_route(start_id: str = Query(..., min_length=1), end_id: str = Query(..., min_length=1), scenario: RainfallScenario = RainfallScenario.MODERATE) -> RouteResponse:
...
def recommend_hospital_for_incident(start_id: str = Query(..., min_length=1), scenario: RainfallScenario = RainfallScenario.MODERATE) -> HospitalRecommendationResponse:
    district = get_district()
    _ensure_location(start_id, district)
...
def recommend_team_for_incident(incident_zone_id: str = Query(..., min_length=1), scenario: RainfallScenario = RainfallScenario.MODERATE, required_specialty: str | None = Query(None, min_length=1)) -> TeamAllocationResponse:
    district = get_district()
