...
from app.models.shelter_recommendation import ShelterCandidate, ShelterExclusion, ShelterRecommendationResponse
...
def recommend_shelter(district: DistrictProfile, start_id: str, scenario: RainfallScenario) -> ShelterRecommendationResponse:
    """Rank reachable shelters using explicit safety, capacity, and distance factors."""
    ...
    for facility in district.shelters:
        ...
        if available_capacity <= 0:
            exclusions.append(ShelterExclusion(shelter_id=facility.id, shelter_name=facility.name, reason="Excluded because no capacity is currently available."))
            continue
        flood_risk = flood_risk_by_zone[facility.zone_id]
        if flood_risk >= _MAX_SAFE_SHELTER_FLOOD_RISK:
            exclusions.append(ShelterExclusion(shelter_id=facility.id, shelter_name=facility.name, reason=f"Excluded because local flood risk is {flood_risk:.1f}/100, above the {_MAX_SAFE_SHELTER_FLOOD_RISK:.0f} safety threshold."))
            continue
        route, route_distance_km = find_safe_route(start_id, facility.id, blocked_road_ids)
        if not route:
            exclusions.append(ShelterExclusion(shelter_id=facility.id, shelter_name=facility.name, reason="Excluded because no safe route remains after flood-blocked roads are removed."))
            continue
        ...
        candidates.append(ShelterCandidate(
            ...
            rationale=f"Safe route is {route_distance_km:.2f} km; {available_capacity} spots are available; local flood risk is {flood_risk:.1f}/100.",
            reasoning_factors=[
                ReasoningFactor(factor="Flood safety", value=f"{flood_risk:.1f}/100 risk", weight=0.50, contribution=safety_score),
                ReasoningFactor(factor="Available capacity", value=f"{available_capacity} / {facility.capacity} spots", weight=0.30, contribution=capacity_score),
                ReasoningFactor(factor="Safe-route distance", value=f"{route_distance_km:.2f} km", weight=0.20, contribution=distance_score),
            ],
        ))

    ranked_shelters = sorted(candidates, key=lambda c: (-c.suitability_score, c.route_distance_km, c.shelter_id))
    if not ranked_shelters:
        return ShelterRecommendationResponse(status="no_suitable_shelter", start_id=start_id, scenario=scenario.value, selected_shelter=None, ranked_shelters=[], excluded_shelters=exclusions, explanation="No shelter satisfies the current capacity, flood-risk, and safe-route constraints.")
    ...
    return ShelterRecommendationResponse(
        status="success", start_id=start_id, scenario=scenario.value, selected_shelter=selected,
        ranked_shelters=ranked_shelters, excluded_shelters=exclusions,
        explanation=(f"{selected.shelter_name} is selected with a {selected.suitability_score:.1f}/100 suitability score. "
                     f"The strongest score contribution is {top_factor.factor.lower()} ({top_factor.contribution:.1f} points), "
                     f"with {selected.available_capacity} available spaces and a {selected.route_distance_km:.2f} km safe route."),
    )
